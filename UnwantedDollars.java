...
public class UnwantedDollars {
    ...
    public static double moneyValue(String money) {
        double dollar = 0, negative = 1;
        for (int i = 0; i < money.length(); i++) {
            char x = money.charAt(i);
            if ('0' <= x && x <= '9') {
                dollar=(dollar*10)+x-'0';
            } else if (x == '.' && i+1 != money.length()) {
                x = money.charAt(++i);
                double fraction=1;
                while ('0' <= x && x <= '9') {
                    x = money.charAt(i);
                    dollar += (x - '0')/(fraction*=10);
        ...
        return dollar * negative;
    }

    public static void main(String[] args) throws IOException {
        debug(moneyValue("$-2.3456"));

...
public class BuyCar {
    ...
    public static int[] nbMonths(double startPriceOld, double startPriceNew, double savingperMonth, double percentLossByMonth) {
        double left = -startPriceNew + startPriceOld;
        int n = 0;
        if (0 > left) {
            left += savingperMonth;
            double losspercent = 1 - percentLossByMonth/100;
            while (0 > left) {
                n++;
                if ((n & 1) == 0) {
                    losspercent -= 0.005;
                }
                startPriceNew *= losspercent;
                startPriceOld *= losspercent;
                left = -startPriceNew + startPriceOld + savingperMonth * n;
            }
        }
        return new int[]{n, (int) round(left)};
    }

    public static void main(String[] args) throws IOException {
        debug(nbMonths(2000, 8000, 1000, 1.5));

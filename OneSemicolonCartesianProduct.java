...
public class OneSemicolonCartesianProduct {
    //    public class Main {
    static Set<int[]> set = new HashSet<>();

    public static java.util.Set<int[]> cartesianProduct(int[][] sets) {
        for (int f = 0; f < sets[0].length; f++) {
            for (int s = 0; s < sets[1].length; s++) {
                for (int l = 0; l < sets[2].length; l++) {
                    set.add(new int[]{sets[0][f],sets[1][s],sets[2][l]});
                }
            }
        }
        return set;

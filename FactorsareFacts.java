...
public class FactorsareFacts {
//public class Main {

    //    static ArrayList<Integer> list = new ArrayList<>();
    static int N = 1000001;
    static boolean primes_bits[] = new boolean[N + 1];
    static int[] primes;

    static void sieveOfEratosthenes() {
        int count = 0;
        for (int i = 2; i < primes_bits.length; i++) {
            if (!primes_bits[i]) {
                for (int j = 2 * i; j < primes_bits.length; j += i) {
                    primes_bits[j] = true;
                }
//                list.add(i);
                count++;
    ...
    static int getCount(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            for (int j = 0; j < primes.length; j++) {
                ...
                if (tmp % primes[j] == 0) {
                    Integer cnt = map.get(primes[j]);
                    if (cnt == null) {
                        map.put(primes[j], 1);
                    } else {
                        cnt++;
                        map.put(primes[j], cnt);
                    }
                    tmp = tmp / primes[j];
                    j = -1;
                }

...
//        public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i < n; i++) {
            int x = sc.nextInt();
            int pre=i-1;
            dp[0][i] = 1 + min(min(dp[0][pre], dp[1][pre]), dp[2][pre]);
            dp[1][i] = (x == 0 || x == 1) ? Integer.MAX_VALUE : min(dp[0][pre], dp[2][pre]);
            dp[2][i] = (x == 0 || x == 2) ? Integer.MAX_VALUE : min(dp[0][pre], dp[1][pre]);
        }
        out.println(min(min(dp[0][--n], dp[1][n]), dp[2][n]) + "");
        close.close();
    }

...
//public class Main {
    ...
    public static int[] lsp(String input) {
        int[] lsp = new int[input.length()];
        int k = 0;
        for (int q = 1; q < input.length(); q++) {
            char target = input.charAt(q);
            while (k > 0 && input.charAt(k) != target) k = lsp[k - 1];
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            ...
            String revBlankStr = (new StringBuilder(str).reverse()).toString() + '?' + str;
            int lsp[] = lsp(revBlankStr);
            int at = 2 * str.length() - lsp[lsp.length - 1] - str.length();
            out.println(str + (new StringBuilder(str.substring(0, at)).reverse()).toString());
        }
    ...
    // 2:41 PM Wed 12 2020 12/16/2020
}
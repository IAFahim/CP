...
//        public class Main {
    ...
    static long bigMod(long a, long p) { // returns (a^p) % m
        long res = 1 % m, x = a % m;
        while (p > 0) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int z = 0; z < till; z++) {
            int i1 = sc.nextInt(), i2 = sc.nextInt(), j1 = sc.nextInt(), j2 = sc.nextInt();
            long x1 = bigMod(2, i2 + 1) - 1;
            x1 = (x1 + m) % m;
            long x2 = bigMod(2, i1) - 1;
            x2 = (x2 + m) % m;
            long y1 = (x1 - x2 + m) % m;
            long x3 = bigMod(3, j2 + 1) - 1;
            x3 = (x3 + m) % m;
            long pp = 500000004;
            x3 = (x3 * (pp) % m) % m;
            long x4 = bigMod(3, j1) - 1;
            x4 = (x4 + m) % m;
            x4 = (x4 * (pp) % m) % m;
            long y2 = (x3 - x4 + m) % m;
            long anser=(y1*y2)%m;

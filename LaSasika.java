...
public class LaSasika {
//            public class Main {
    public static int primeFactors(int n) {
        ArrayList<Integer> prime_factor_int = new ArrayList<>();
        // Print the number of 2s that divide n
        while (n % 2 == 0) {
            prime_factor_int.add(2);
            n /= 2;
        }
        ...
        int m=1,at=0,count=1;
        for (int i = 0; i < prime_factor_int.size(); i++) {
            if(at==prime_factor_int.get(i)){
                count++;
            }else {
                if((count & 1) == 0 ){
                    m*=at;

...
//        public class Main {
    public static void main(String[] args) throws IOException {
        ...
        for (int z = 0; z < till; z++) {
            int n=sc.nextInt();
            int a=0,b=0,c=0,d=Integer.MAX_VALUE,e=Integer.MAX_VALUE,f=Integer.MAX_VALUE;
            for (int i = 0; i < n; i++) {
            int x1=sc.nextInt(),y1=sc.nextInt(),z1=sc.nextInt();
            int x2=sc.nextInt(),y2=sc.nextInt(),z2=sc.nextInt();
            a=Math.max(a,x1);b=Math.max(b,y1);c=Math.max(c,z1);
            d=Math.min(d,x2);e=Math.min(e,y2);f=Math.min(f,z2);
            }
            out.println("Case "+(z+1)+": "+((d>a&&e>b&&f>c)?(d - a) * (e - b) * (f - c):0));
        }
        close.close();
    }

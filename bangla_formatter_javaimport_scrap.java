...
public class bangla_formatter_javaimport_scrap {
    //    public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            ...
            for (i = 0; i < str.length && !str[i].equals("="); i++) {
                name.append(str[i]);
            ...
            for (; i < str.length; i++) {
                ...
                if (str[i].charAt(str[i].length() - 1) == ';') {
                    frag.deleteCharAt(frag.length() - 1);
                    break;
                }
            }
            frag.append("\',");
            ...
            i += 2;
            for (; i < str.length; i++) {
                if(str[i].charAt(0)=='(')break;
                if (str[i].charAt(str[i].length() - 1) == ',') {
                    example.append(str[i]);
    ...
    static class Sc {
        ...
        public String readLine() throws IOException {
            byte[] buf = new byte[900]; // line length
            int cnt = 0, c;
            while ((c = read()) != -1) {
                if (c == '\n')
                    break;

...
//        public class Main {
    ...
    static int runDFS(int root,boolean visited[]) {
        int count=1;
        visited[root] = true;
        for (int i : adj.get(root)) {
            if (!visited[i]) {
               count+=runDFS(i,visited);
            }
        }
        return count;
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        clearGraph();
        for (int i = 0; i < e; i++) {
            int firstVertex = sc.nextInt(), endVertex = sc.nextInt();
            addEdge(firstVertex, endVertex);
        }
        boolean vistied[] = new boolean[SIZE];
        List<Integer> countries = new ArrayList<Integer>();
        int run_count[]=new int[100000];
        for (int i = 0; i < SIZE; i++) {
            if (!vistied[i]) {
                countries.add(runDFS(i,vistied));
            }
        }
        long combination=0;
        int sum=0;

...
//        public class Main {
    ...
    static void rundfs(int root) {
        grid[root] = true;
        for (int i = 0; i < adj.get(root).size(); i++) {
            if(!grid[adj.get(root).get(i)]) rundfs(adj.get(root).get(i));
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int z = 0; z < till; z++) {
            ...
            while (sc.hasNextLine()) {
                ...
                int first = current.charAt(0) - 'A', last = current.charAt(1) - 'A';
                addEdge(first, last);
            }
//            printAdjacency();
            int discontus = 0;
            ...
            out.println(discontus+((till==z+1)?"":"\n"));
        }
        close.close();
    }

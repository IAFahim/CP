...
//        public class Main {
    public static void main(String[] args) throws IOException {
        ...
        for (int z = 0; z < till; z++) {
            double ox=sc.nextDouble(),oy=sc.nextDouble();
            double ax=sc.nextDouble(),ay=sc.nextDouble();
            double bx=sc.nextDouble(),by=sc.nextDouble();
            double redious=Math.sqrt((ox-ax)*(ox-ax)+(oy-ay)*(oy-ay));
            double distanceAB=Math.sqrt((bx-ax)*(bx-ax)+(by-ay)*(by-ay));
            double angle=(Math.asin((distanceAB/(2*redious)))*360.0)/Math.PI;
            double answer=((redious*angle)/360)*(2*Math.PI);

...
public class FriendshipMatters {
    //    public class Main {
...
//////        return names[x];
//    }
    public static class faluda{
        int n;
    }
    static String names[];

    public static void main(String[] args) throws IOException {
        int name_count = sc.nextInt(), que = sc.nextInt();
        HashMap<String, Integer> names = new HashMap<>();
        for (int i = 0; i < name_count; i++) {
            names.put(sc.nextString(), i);
        }
        for (int i = 0; i < que; i++) {
            int pick = sc.nextInt();
            String first = sc.nextString();
            String second = sc.nextString();
            if (pick == 1) {

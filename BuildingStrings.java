...
//        public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int z = 0; z < till; z++) {
            ...
            char temp[] = sc.nextString().toCharArray();
            int arr[][] = new int[26][1];
            for (int i = 0; i < 26; i++) {
                arr[i][0]=Integer.MAX_VALUE;
            }
            for (int i = 0; i < n; i++) {
                int val = (int) (temp[i] - '0');
                int sval = (int) (str[i] - 'a');
                arr[sval][0] = Math.min(val, arr[sval][0]);
            }
//            debug(arr);
            char st[] = sc.nextString().toCharArray();
            int sum = 0;
            boolean flaw = false;
            for (int i = 0; i < len; i++) {
                int sval = (int) (st[i] - 'a');
                if (arr[sval][0] == Integer.MAX_VALUE) {
                    flaw = true;
                    break;
                }
                sum += arr[sval][0];

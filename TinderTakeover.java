...
//        public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        ArrayList<Long> d = getDivisors(n);
        debug(d);
        long a[] = new long[2], min = Long.MAX_VALUE;
        for (int i = 0; i < d.size(); i++) {
            for (int j = i + 1; j < d.size(); j++) {
                if (min > abs(d.get(i) - d.get(j)) && d.get(i) * d.get(j) == n) {
                    a[0] = d.get(i);
                    a[1] = d.get(j);
                    min=abs(d.get(i) - d.get(j));
                    debug(min);
                }
            }
        }

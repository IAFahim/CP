...
public class TheWalker {
    ...
    public static int[] solve(int a, int b, int c, int alpha, int beta, int gamma) {
        double red = PI / 180;
        double deg[] = new double[]{alpha * red, beta * red, gamma * red};
        double dis[] = new double[]{a, b, c};
        double x = dis[0] * cos(deg[0]), y = dis[0] * sin(deg[0]);
        x -= dis[1] * sin(deg[1]);
        y += dis[1] * cos(deg[1]);
        x -= dis[2] * cos(deg[2]);
        y -= dis[2] * sin(deg[2]);
        double ocDis = hypot(x, y), ocDeg = atan2(y, x) * 180 / PI;
        double min = (ocDeg - (int) ocDeg) * 60;
        double sec = (min - (int) min) * 60;
        int answer[] = new int[]{(int) round(ocDis), (int) ocDeg, (int) min, (int) sec};
        return answer;
    }

    public static void main(String[] args) throws IOException {
        debug(solve(12, 20, 18, 45, 30, 60));

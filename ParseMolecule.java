    ...
    public static Map<String, Integer> getAtoms(String formula) {
        ...
        while (i < formula.length()) {
            if ('A' <= formula.charAt(i) && formula.charAt(i) <= 'Z') {
                ...
                while (i < formula.length() && 'a' <= formula.charAt(i) && formula.charAt(i) <= 'z') {
                    str.append(formula.charAt(i++));
                }

                StringBuilder num = new StringBuilder();
                while (i < formula.length() && '0' <= formula.charAt(i) && formula.charAt(i) <= '9') {
                    num.append(formula.charAt(i++));
                }
                int n=1;
                if(num.length()!=0)
                n = Integer.parseInt(num.toString());
                String at = str.toString();
                Integer x = store.get(at);
                if (x == null) {
                    x = 1;
                }
                store.put(at, n*x);

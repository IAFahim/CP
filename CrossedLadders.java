...
//        public class Main {
    ...
    static double binarySearch(double l,double h){
        double mid=l+(h-l)/2;
        if(h-l<(double) 1e-7){
            return mid;
        }else {
            calc(mid);
            if(p+q<r)return binarySearch(mid,h);
            else return binarySearch(l,mid);
        }
    }
    static double p,q,r;
    static double x,y,c;
    public static void main(String[] args) throws IOException {
        ...
        out.println("Case "+(z+1)+": "+binarySearch(0,min(x,y)));

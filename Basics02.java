...
public class Basics02 {
    ...
    public static String calculateArray(String stringArray) {
        String result = "";
        int sum = 0, at = 0, n = stringArray.length(), count = 0;
        boolean neg = false;
        for (int i = 0; i < n; i++) {
            char x = stringArray.charAt(i);
            if (x == '-') {
                neg = !neg;
            }
            if ('0' <= x && '9' >= x) {
                at = x - '0';
                while (++i < n && (x = stringArray.charAt(i)) != ';') {
                    if ('0' <= x && '9' >= x) {
                        at = (at * 10) + x - '0';
                    }
                }
    ...
    public static void main(String[] args) throws IOException {
        int[] input = new int[]{};
        debug(printLargestAndSmallest(input));
        close.close();
    }

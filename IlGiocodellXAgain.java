...
public class IlGiocodellXAgain {
//public class Main {
    ...
    static int Y = 0, X = 0;
    static boolean black_reached = false;
    ...
    static void run_dfs(int y, int x) {
        ...
        for (int i = 0; i < dx.length; i++) {
            ...
            if (-1 < y_temp && -1 < x_temp && y_temp < Y && x_temp < X && !grid_visited[y_temp][x_temp] && grid[y_temp][x_temp] == 'b') {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            ...
            for (int x = 0; x < X; x++) {
                if (!grid_visited[0][x] && grid[0][x] == 'b') {
                    run_dfs(0, x);
                }
            }
            out.println((z++) + " " + (black_reached ? "B" : "W"));
            black_reached = false;

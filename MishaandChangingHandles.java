...
//        public class Main {
    ...
    public static class Node{
        ...
        public Node (String head,String tail){
            this.firstName=head;
            this.secondName=tail;
        }
    }
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            Node node=new Node(sc.nextString(),sc.nextString());
            int j=0;
            for (j = 0; j < nodes.size(); j++) {
                if(nodes.get(j).secondName.equals(node.firstName)){
                    nodes.get(j).secondName=node.secondName;
                        break;
                }
            }if(j==nodes.size())
            nodes.add(node);
        ...
        for (int i = 0; i < nodes.size(); i++) {
            out.println(nodes.get(i).firstName+" "+nodes.get(i).secondName);
        }
        close.close();
    }

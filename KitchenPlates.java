...
//        public class Main {
    ...
    static String runBFS() {
        ...
        while (!store.isEmpty()) {
            int root = store.poll();
            str += Character.toString(root + 'A');
            for (int i = 0; i < 5; i++) {//this line was helped by other
                if(grid[root][i]==1){
                    count[i]--;
                    if(count[i]==0){
                        store.add(i);
    ...
    public static void main(String[] args) throws IOException {
        for (int i = 0; i < 5; i++) {
            ...
            if (str.charAt(1) == '>') {
                grid[str.charAt(2) - 'A'][str.charAt(0) - 'A']++;
                count[str.charAt(0) - 'A']++;
            } else {
                grid[str.charAt(0) - 'A'][str.charAt(2) - 'A']++;
                count[str.charAt(2) - 'A']++;
            }
        }
        String str=runBFS();
        out.println(str.length()==5?str:"impossible");
//        printAdjacency();
//        debug(count);
        close.close();
    }
